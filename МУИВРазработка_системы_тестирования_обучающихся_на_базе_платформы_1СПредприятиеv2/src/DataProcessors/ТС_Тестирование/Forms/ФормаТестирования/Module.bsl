...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТС_Студенты.ПереходитьКСледующемуПриВыборе,
	|	ТС_Студенты.НеВыделятьОтмеченныеТесты,
	|	ТС_Студенты.ОткрыватьФормуТестированияВНовомОкне,
	|	ТС_Студенты.ШиринаФормыТестирования,
	|	ТС_Студенты.НеВыделятьОтмеченныеТестыТолькоДляВебКлиента,
	|	ТС_Студенты.ОткрыватьФормуТестированияВНовомОкнеТолькоДляВебКлиента,
	|	ТС_Студенты.ШиринаФормыТестированияТолькоДляВебКлиента,
	|	ТС_Студенты.ПроверятьТестПриВыборе
	|ИЗ
	|	Справочник.ТС_Студенты КАК ТС_Студенты
	|ГДЕ
	|	ТС_Студенты.Ссылка = &Пользователь";
...
Процедура ПриОткрытии(Отказ)
	...
	#Если НЕ ВебКлиент Тогда
		...
		КонецЕсли;
		ЭтаФорма.ОткрытВВебКлиенте = Истина;
	#КонецЕсли
...
Процедура СледующийТест(Команда)
	...
	Если Элементы.ГруппаСтраницы.ПодчиненныеЭлементы.Получить(НомерТекущейСтраницы+1) <> Неопределено Тогда
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаСтраницы.ПодчиненныеЭлементы.Получить(НомерТекущейСтраницы+1);
	КонецЕсли;
...
Процедура ПроверитьТест(Команда)
	...
	флОтветВерен = Ложь;
	Для Сч = НомерНачальнойСтроки ПО Объект.Тесты.Количество()-1 Цикл
...
Процедура СформироватьНаборТестов()
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.НаборТестов) Тогда
		...
		|	Справочник.ТС_Тесты.Подтверждение КАК ТС_ТестыПодтверждение
		|ГДЕ
		|	ТС_ТестыПодтверждение.Ссылка В ИЕРАРХИИ
		|			(ВЫБРАТЬ
		|				ВТНаборыТестов.Тест
		|			ИЗ
		|				ВТНаборыТестов)
		...
		|	РегистрСведений.ТС_ИзбранныеТесты КАК ТС_ИзбранныеТесты
		|ГДЕ
		|	ТС_ИзбранныеТесты.Тест В ИЕРАРХИИ
		|			(ВЫБРАТЬ
		|				ВТНаборыТестов.Тест
		|			ИЗ
		|				ВТНаборыТестов)
	...
	Пока ВыборкаПоТесту.Следующий() Цикл
		...
		Если Не ВыборкаПоТесту.ПеремешиватьОтветы Тогда
...
Процедура ОтобразитьТестыНаформе()
	...
	Для Каждого Строка ИЗ Объект.Тесты Цикл
		...
		Если ТекТест <> ПредыдущийТест Тогда		
			...
			НоваяСтраница = Элементы.Добавить("Страница" + Строка(Строка.НомерСтроки),Тип("ГруппаФормы"),Элементы.ГруппаСтраницы);
			...
			Если ЗначениеЗаполнено(Строка.Изображение) Тогда
				...
				НовоеИзображение.ПутьКДанным = "Объект.Тесты[" + Строка(Строка.НомерСтроки-1) + "].ИзображениеСтрока";
		...
		НовыйОтвет = ЭтаФорма.Элементы.Добавить(
			"Ответ" + Строка(Строка.НомерСтроки),Тип("ПолеФормы"),НоваяСтраница);
		НовыйОтвет.Заголовок = Строка(НомерВопроса) + ") " + Строка.Ответ;
		НовыйОтвет.ПутьКДанным = "Объект.Тесты[" + Строка(Строка.НомерСтроки-1) + "].Выбор";
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ЭтаФорма.РежимТестирования 
		...
		Если Этаформа.ОткрытВВебКлиенте Тогда
